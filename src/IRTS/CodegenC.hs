...
toC f code
    = -- "/* " ++ show code ++ "*/\n\n" ++
      "void " ++ cname f ++ "(VM* vm, VAL* oldbase) {\n" ++
                 ...
                 src ++ "}\n\n"
            where
                (a, s, src) = runRWS (bcc code) 0
                                    (CS { level = 1, counter = 0, subfns = [] })
...
bcc ((CASE True r code def):xs)
  ...
  where
    showCode bc = do
        i <- get
        let (a, s, w) = runRWS (bcc bc) 0 (i { level = (level i)+1 })
        tell $ "{\n" ++ w ++ indent (level i) ++ "}\n"
...
bcc ((CASE safe r code def):xs) = do
  ...
  where
    ...
    showCase (t, bc) = do
        is <- get
        let i = level is
        let (a, s, w) = runRWS (bcc bc) 0 (is { level = i+1 })
    ...
    showDef (Just c) = do
        is <- get
        let i = level is
        let (a, s, w) = runRWS (bcc c) 0 (is { level = (level is)+1 })
